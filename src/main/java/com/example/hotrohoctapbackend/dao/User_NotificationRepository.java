...
public interface User_NotificationRepository extends JpaRepository<User_Notification, Integer> {
    ...
    @Query("SELECT noti FROM User_Notification noti WHERE noti.account.id = :accountId")
    Page<User_Notification> findNotificationByAccountId(Integer accountId, Pageable pageable);
